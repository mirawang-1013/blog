...
这个项目还有很多可以迭代的方向，接下来我计划做两件事：

### 1. 大规模数据检索优化

目前的暴力 cosine 搜索在笔记量小时没问题，但数据量大了会有瓶颈。下一步计划：

- **用 FAISS 替换暴力搜索** — 通过 ANN（近似最近邻）索引，把检索从 O(n) 降到 O(log n)，支撑几十万条数据
- **优化切分策略** — 当前按 `\n\n` 切段落太粗糙，计划加入 overlap（chunk 间重叠 50-100 字防止边界丢信息）和 semantic chunking
- **加入 Rerank** — 先粗筛 top 20，再用 LLM 精排到 top 3，提升检索质量

### 2. 多格式文档支持

目前只能读 Markdown，但实际笔记系统里经常有 PDF、Word 等格式。计划：

- **PDF 支持** — 用 `PyMuPDF` 或 `pdfplumber` 提取文字，处理表格和多栏排版
- **统一 loader 接口** — 不管什么格式，最终都输出 `list[str]`，后面的 embedding 和检索完全不用改
- **长期考虑** — 引入 `Unstructured.io` 或 LangChain DocumentLoaders 来处理更多格式（HTML、PPT、扫描件 OCR 等）

核心设计原则不变：**格式解析和检索逻辑解耦，新增格式只需要加一个 loader。**
